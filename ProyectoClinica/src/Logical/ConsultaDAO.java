...
public class ConsultaDAO extends BaseDAO {
    ...
    public boolean insertarConEnfermedades(Consultas consulta,
                                           ...
                                           String idHistorial) {
        // FIX: se usa try-with-resources para garantizar que la conexión
        // siempre se cierre, incluso si ocurre una excepción
        try (Connection con = getConexion()) {
            if (con == null) {
                System.out.println("Error: conexión null en insertarConEnfermedades");
                return false;
            }
            con.setAutoCommit(false);
            try {
                // ── INSERT CONSULTA principal ────────────────────────
                try (PreparedStatement ps = con.prepareStatement(
                    "INSERT INTO CONSULTA (Id_Consulta,Fecha_Consulta,Diagnostico,"
                    + "Id_Historial,cedula,cedula_paciente,Id_Cita) VALUES (?,?,?,?,?,?,?)")) {
                ...
                // FIX: si algo falla, se hace rollback para no dejar datos a medias
                ...
                // Siempre restaurar autoCommit antes de que el try-with-resources cierre la conexión
                try { con.setAutoCommit(true); } catch (SQLException ex) { /* ignorar */ }
            }

        } catch (SQLException e) {
            System.out.println("Error al abrir conexión en insertarConEnfermedades: " + e.getMessage());
            return false;
        }
    }
