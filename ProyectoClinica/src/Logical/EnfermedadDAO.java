    ...
    public ArrayList<Enfermedad> listarTodos() {
        ...
        try (Statement st = con.createStatement();
             ResultSet rs = st.executeQuery(
                ...
                + "g.Id_Gravedad,g.Gravedad FROM ENFERMEDAD e"
                + " LEFT JOIN GRAVEDAD_ENFERMEDAD g ON e.Id_Gravedad=g.Id_Gravedad"
                + " ORDER BY e.NombreEnfermedad")) {
            while (rs.next()) {
                Gravedadenfermedad grav = null;
                if (rs.getString("Id_Gravedad") != null)
                    grav = new Gravedadenfermedad(rs.getString("Id_Gravedad"), rs.getString("Gravedad"));
