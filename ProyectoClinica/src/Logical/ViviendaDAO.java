...
public class ViviendaDAO extends BaseDAO {
    ...
    public boolean insertar(Vivienda v) {
        return ejecutar(
            "INSERT INTO VIVIENDA (Id_Vivienda,Direccion) VALUES (?,?)",
            v.getIdVivienda(),
            v.getDireccion()
        );
    }
    ...
    public ArrayList<Vivienda> listarTodos() {
        ...
        try (Connection con = getConexion();
             ...
             ResultSet rs = st.executeQuery(
                 "SELECT Id_Vivienda,Direccion FROM VIVIENDA")) {
            ...
            System.out.println("Error ViviendaDAO: " + e.getMessage());
        }
    ...
    public Vivienda buscarPorId(String id) {
        try (Connection con = getConexion();
             PreparedStatement ps = con.prepareStatement(
                 "SELECT Id_Vivienda,Direccion FROM VIVIENDA WHERE Id_Vivienda=?")) {
    ...
    public boolean actualizar(Vivienda v) {
        return ejecutar(
            "UPDATE VIVIENDA SET Direccion=? WHERE Id_Vivienda=?",
            v.getDireccion(),
            v.getIdVivienda()
        );
    }
